...
class GeneralTransformerDecoder(torch.nn.Module):
    ...
    def inference(self, start_feature: Tensor, memory: Tensor, start_mask: Tensor = None, prob_mask: Tensor = None):
        ...
        while(t+1<self.max_length and len(running_seqs)>0):
            ...
            last_input = torch.nn.functional.sigmoid(self.linear(input_))[:, -1, :]
            # concat
            input_sequence = torch.cat([input_sequence, last_input.unsqueeze(dim=1)], dim=1)
            # save next input
            ...
            if input_mask is not None:
                if self.use_prob_mask:
                    ...
                    last_mask =  torch.nn.functional.sigmoid(self.linear_m(input_))[:, -1, :]
                    # save next input
                    input_mask = torch.cat([input_mask, last_mask.unsqueeze(dim=1)], dim=1)
