...
from utils.train_utils import to_var, sample_start_feature_time_mask_gender_race, sample_mask_from_prob, mimic3_model_inference as model_inference
...
def train_model(args, datasets, prob_mask, **kwargs):
    if not args.test:
        ...
        if args.dp_sgd == True: # opt_dx and opt_dz access origin data too?
            ...
            epsilon = moments_accountant.epsilon(len(datasets['train']), args.batch_size, args.noise_multiplier, args.epochs, args.delta)
    ...
    Imi.eval()
    G.eval()
    model_generation(args, G, Imi, args.batch_size, args.feature_size, **kwargs)
    #
    Dec = models["Trans"].decoder
    Dec.eval()
    model_generation(args, G, Dec, args.batch_size, args.feature_size, prefix=args.model_type+"_pub", **kwargs)


def model_generation(args, G_0, G_1, batch_size, fea_size, path=None, prefix=None, **kwargs,):
    if path is None:
    ...
    for i in range(args.gendata_size//args.batch_size):
        ...
        Pimi, Mimi = model_inference(args, G_1, zgen, batch_size, fea_size, **kwargs)
...
def model_evaluation(args, models, opts, lrs, data_loader, prob_mask, split, log_file, **kwargs):
    ...
    for iteration, batch in enumerate(data_loader):
        ...
        '''
        batch_size, max_length, fea_size = src_tempo.shape
        #start_feature = torch.tensor(np.ones((batch_size, fea_size)))
        #start_feature = torch.tensor(np.random.uniform(size=(batch_size, fea_size)))
        start_feature = torch.tensor(np.random.uniform(size=(batch_size, fea_size))*0.2 + 0.4)
        start_mask = torch.tensor(np.ones((batch_size, fea_size)))
        ...
        if args.no_mask:
            ...
            Pgen, Mgen = Trans.decoder.inference(start_feature=start_feature, start_mask=start_mask, prob_mask=prob_mask, memory=zgen, **kwargs)
            Pimi, Mimi = Imi.inference(start_feature=start_feature, start_mask=start_mask, prob_mask=prob_mask, memory=zgen, **kwargs)
        else:
            Pgen, Mgen = Trans.decoder.inference(start_feature=start_feature, start_mask=start_mask, memory=zgen, **kwargs)
            Pimi, Mimi = Imi.inference(start_feature=start_feature, start_mask=start_mask, memory=zgen, **kwargs)
